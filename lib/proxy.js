...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  if (hostPort === null) {
    ...
    if (headers['upstream']) {
      try {
        const [_, username, password, host, port] = Buffer.from(headers['upstream'], 'base64').toString().match(/http\:\/\/(.+)\:(.+)@(.+):(.+)/);
        upstreamProxy = {};
        upstreamProxy.username = username;
        upstreamProxy.password = password;
        upstreamProxy.host     = host;
        upstreamProxy.port     = parseInt(port);
        upstreamProxy.auth     = `Basic ${Buffer.from(`${username}:${password}`).toString('base64')}`;
      } catch {}
      delete headers['upstream'];
    }
    ...
    if (upstreamProxy) {
      headers['Proxy-Authorization'] = upstreamProxy.auth;
      ctx.proxyToServerRequestOptions.host = upstreamProxy.host;
      ctx.proxyToServerRequestOptions.port = upstreamProxy.port;
      ctx.proxyToServerRequestOptions.path = `${hostPort.host}:${hostPort.port}/${ctx.clientToProxyRequest.url}`;
    }
